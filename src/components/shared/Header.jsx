      ...
      <div className="max-w-6xl mx-auto flex justify-between items-center">
        ...
        <div className="hidden sm:flex flex-1 justify-center items-center space-x-4">
          {navsData.map((nav) => (
            <Link
              key={nav.name}
              href={nav.path}
              className={`text-xl text-white font-medium hover:text-pink-400  ${
                pathName === nav.path ? "!text-pink-500 !underline" : ""
              }`}
            >
              {nav.name}
            </Link>
        ...
        <div className="hidden sm:flex items-center space-x-4">
          {user ? (
            ...
            <Link
              href="/login"
              className="bg-pink-500 hover:brightness-110 duration-300 text-white px-3 py-2 rounded font-medium"
            >
              Login
            </Link>
          )}
        ...
        <div className="flex sm:hidden items-center">
          <button
            ...
            className="inline-flex items-center justify-center p-2 rounded-md text-gray-400 hover:text-gray-800 hover:bg-black focus:outline-none focus:bg-black focus:text-gray-800"
          >
            {isOpen ? (
